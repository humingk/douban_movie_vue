<template>
    <div id="top">
        <div id="db-nav-movie" class="nav glassbox radis">
            <div class="nav-wrap">
                <div class="nav-primary">
                    ...
                    <div class="nav-search">
                        <div class="inp">
                            ...
                            <!--使用blur自动关闭搜索提示框-->
                            <!--使用focus自动打开搜索提示框-->
                            <input id="inp-query" ref="searchInput" size="22" maxlength="60" placeholder="搜索电影、电视剧、影人"
                                   autocomplete="off"
                                   class="radis"
                                   style="background: rgba(255, 248, 220, 0.8);"
                        ...
                        <div id="search_suggest" style="position:fixed;margin:31px 300px 0 90px;width: 445px;">
                            <transition-group name="itemfade" tag="ul" mode="out-in"
                                              v-cloak :modal-append-to-body="false">
                                <li v-for="(item,index) in searchResult"
                                    v-if="item.name!=''"
                                    :style="radisForSearch(index)+' background: rgba(255, 255, 255, 0.9);border-top: 1px solid;border-color: #c7cbff;height: auto;'"
                                    ...
                                    <span style="color: #f7097a;font-size: 18px;float: right;"><i>{{item.rate=='11'||item.rate=='0'?'':item.rate}}&nbsp;</i></span><br>
                    ...
                    <div id="github">
                        <a title="Welcome Star/Fork" href="https://github.com/humingk/douban_movie"
                           target="_blank">
                            <svg viewBox="0 0 250 250" style="position:absolute;top: -23px;right: 36px;padding: 10px 0 0 0"
                                 width="86px" height="86px" aria-hidden="true">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 智能提示圆角
            radisForSearch: function (index) {
                if (index == 0) {
                    return "border-radius: 8px 8px 0 0;";
                } else if (index == this.searchResult.length - 1) {
